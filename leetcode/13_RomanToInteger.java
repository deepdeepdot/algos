...
class Solution {
    ...
    Map<Character, Integer> getRomanMap() {
    ...
    int getRomanDigitValue(char letter) {
        Map<Character, Integer> mapping = getRomanMap();
        int val = mapping.get(letter) != null ? mapping.get(letter) : 0;
        return val;
    }

    boolean greater(char left, char right) {
        String digits = "MDCLXVI";

        int l = digits.indexOf(left);
        int r = digits.indexOf(right);
        if (l >= 0 && r >= 0) {
            return (l < r);
        }
